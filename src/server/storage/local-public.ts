...
import fs from "fs";
import type { StorageProvider, StorageTarget } from "./provider";
...
export class LocalPublicStorageProvider implements StorageProvider {
  ...
  async prepareTarget(extension: string): Promise<StorageTarget> {
    const dir = path.join(this.publicDir, this.subDir);
    await fs.promises.mkdir(dir, { recursive: true });
    const filename = generateUniqueName("scene", extension);
    const filePath = path.join(dir, filename);
    const publicUrl = path.posix.join("/", this.subDir, filename);
