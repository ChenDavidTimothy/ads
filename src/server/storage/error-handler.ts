...
export class StorageErrorHandler {
  ...
  static createBucketError(bucketName: string, reason: string, accessible: boolean): StorageError {
    const code = accessible ? StorageErrorCode.BUCKET_ACCESS_DENIED : StorageErrorCode.BUCKET_NOT_FOUND;
    return this.createError(
      code,
  ...
  static createUploadError(bucket: string, remoteKey: string, reason: string, attempt: number): StorageError {
    return this.createError(
