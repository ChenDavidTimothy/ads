...
import type { CachedAsset, JobManifest } from '@/server/rendering/asset-cache-manager';
...
const isCachedAsset = (value: unknown): value is CachedAsset => {
  ...
  const { assetId, localPath, contentHash, size, contentType, verified } = value;
  return (
...
const isJobManifest = (value: unknown): value is JobManifest => {
  ...
  if (typeof value.totalBytes !== 'number' || typeof value.createdAt !== 'string') {
    return false;
  }
  if (
...
describe('AssetCacheManager', () => {
  ...
  it('downloads assets, records metrics, and persists manifest', async () => {
    ...
    const manifestPath = path.join(jobCacheDir, 'manifest.json');
    const manifestRaw: unknown = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
    expect(isJobManifest(manifestRaw)).toBe(true);
    if (!isJobManifest(manifestRaw)) {
      throw new Error('Manifest format mismatch');
    }
    expect(manifestRaw.assets['asset-1']?.localPath).toBe(cachedAsset.localPath);
  });
});
