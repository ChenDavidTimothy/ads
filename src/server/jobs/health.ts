...
export async function checkJobSystemHealth(): Promise<JobSystemHealth> {
  ...
  if (!eventHealth.listenerConnected && !eventHealth.publisherConnected) {
    ...
    !eventHealth.listenerConnected ||
    !eventHealth.publisherConnected
  ) {
    eventStatus = "degraded";
    if (!eventHealth.listenerConnected) {
      recommendations.push(
        "Event listener is down - clients may not receive completion events",
      );
    }
    if (!eventHealth.publisherConnected) {
      recommendations.push(
        "Event publisher is down - job completion notifications may fail",
      );
    }
  }
  ...
  try {
    ...
    recommendations.push(
      "Cannot access queue statistics - check database connectivity",
    );
  }
  ...
  if (overall === "unhealthy") {
    recommendations.unshift(
      "Job system is unhealthy - immediate attention required",
    );
  } else if (overall === "degraded") {
    recommendations.unshift(
      "Job system is degraded - some functionality may be impacted",
    );
  }
