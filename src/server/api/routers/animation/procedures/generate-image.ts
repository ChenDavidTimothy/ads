...
export const generateImageProcedure = protectedProcedure
  ...
  .mutation(async ({ input, ctx }: { input: GenerateSceneInput; ctx: AnimationTRPCContext }) => {
    try {
      ...
      const executionContext = await engine.executeFlow(reactFlowNodesForValidation, backendEdges, {
        requireScene: false,
      });

      const frameNodes = reactFlowNodesForValidation.filter((node) => node.type === 'frame');
      ...
      const result = await generateImageJobsWithAssetCache(scenePartitions, ctx.user!.id);
