...
import { isDomainError, NoValidScenesError, UserJobLimitError } from '@/shared/errors/domain';
...
export const generateSceneProcedure = protectedProcedure
  ...
  .mutation(async ({ input, ctx }: { input: GenerateSceneInput; ctx: AnimationTRPCContext }) => {
    try {
      ...
      const executionContext = await engine.executeFlow(reactFlowNodesForExecution, backendEdges);
