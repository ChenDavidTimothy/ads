...
export function createAssetLifecycleService({
  ...
  async function deleteAsset({ userId, input }: DeleteAssetParams): Promise<DeleteAssetResult> {
    ...
    if (typeof asset.bucket_name !== 'string' || typeof asset.storage_path !== 'string') {
      throw new AssetsServiceError('INTERNAL_SERVER_ERROR', 'Invalid asset data');
    }
    ...
    if (storageError) {
      throw new AssetsServiceError('INTERNAL_SERVER_ERROR', 'Failed to delete file from storage', {
        cause: storageError,
      });
    }
    ...
    if (dbError) {
      throw new AssetsServiceError('INTERNAL_SERVER_ERROR', 'Failed to delete asset record', {
        cause: dbError,
      });
    }
  ...
  async function moveRenderJobToAssets({
    ...
    if (jobError || !renderJob?.output_url) {
      throw new AssetsServiceError('NOT_FOUND', 'Render job not found or not completed', {
        cause: jobError,
      });
    }

    if (typeof renderJob.output_url !== 'string') {
      throw new AssetsServiceError('INTERNAL_SERVER_ERROR', 'Invalid output URL format');
    }

    const { bucket, filePath, mimeType, extension } = parseSignedUrl(renderJob.output_url);

    if (!bucket || !filePath) {
      throw new AssetsServiceError('BAD_REQUEST', 'Invalid output URL format');
    }

    const filename = filePath.split('/').pop();
    const parentDir = filePath.includes('/') ? filePath.slice(0, filePath.lastIndexOf('/')) : '';

    const listResult = await supabase.storage.from(bucket).list(parentDir);
    ...
    if (fileError || !dirEntries || dirEntries.length === 0 || !filename) {
      throw new AssetsServiceError('NOT_FOUND', 'File not found in storage', {
        cause: fileError,
      });
    }
    ...
    if (!fileData) {
      throw new AssetsServiceError('NOT_FOUND', 'File metadata not found');
    }
    ...
    const assetName = input.originalName ?? `Generated_${nowFn().getTime()}.${extension}`;
    ...
    if (insertError) {
      ...
      throw new AssetsServiceError('INTERNAL_SERVER_ERROR', 'Failed to save asset record', {
        cause: insertError,
      });
    }
