...
export function createAssetUploadService({
  ...
  async function prepareUpload({
    ...
    if (!validateMimeType(input.mimeType)) {
      throw new AssetsServiceError('BAD_REQUEST', `Unsupported file type: ${input.mimeType}`);
    }

    if (!validateFileSize(input.fileSize, input.mimeType)) {
      throw new AssetsServiceError('BAD_REQUEST', `File size exceeds limit for ${input.mimeType}`);
    }
    ...
    if (error || !signedUrl) {
      throw new AssetsServiceError('INTERNAL_SERVER_ERROR', 'Failed to create upload URL', {
        cause: error,
      });
    }
    ...
    if (insertError) {
      throw new AssetsServiceError('INTERNAL_SERVER_ERROR', 'Failed to register asset', {
        cause: insertError,
      });
    }
    ...
    if (asset.mime_type.startsWith('image/') && asset.mime_type !== 'image/svg+xml') {
      try {
        const signed = (await supabase.storage
          ...
          .createSignedUrl(asset.storage_path, 300)) as DatabaseResponse<StorageResponse>;

        if (!signed?.data?.signedUrl) {
          log.warn?.(`Could not create signed URL for dimension processing: ${asset.storage_path}`);
        } else {
          ...
          if (metadata.width && metadata.height) {
            ...
            if (updateError) {
              const message = (updateError as { message?: string })?.message ?? 'unknown';
              log.warn?.(`Failed to update image dimensions for ${input.assetId}: ${message}`, {
                error: updateError,
              });
            } else {
              log.info?.(
