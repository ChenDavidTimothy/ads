...
const createQueryBuilder = (result: QueryResult): QueryBuilder => {
  ...
  builder.select = vi.fn(
    (_columns: string, _options: { count: "exact" }) => builder,
  );
...
type StorageBucket = {
  ...
  createSignedUrl: (
    path: string,
    expiresIn: number,
  ) => Promise<DatabaseResponse<StorageResponse>>;
};
...
describe("createAssetCatalogService", () => {
  it("lists assets with signed URLs", async () => {
    ...
    const builder = createQueryBuilder({ data: [asset], error: null, count: 1 });

    const storageBucket: StorageBucket = {
      list: vi
        .fn<(path?: string) => Promise<{ data: StorageFileInfo[] | null; error: unknown }>>(async () => ({
  ...
  it("filters assets missing in storage", async () => {
    ...
    const storageBucket: StorageBucket = {
      ...
      createSignedUrl: vi
        .fn<(path: string, expiresIn: number) => Promise<DatabaseResponse<StorageResponse>>>(
