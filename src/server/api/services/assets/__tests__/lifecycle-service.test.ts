...
describe("createAssetLifecycleService", () => {
  it("deletes asset and updates quota", async () => {
    ...
    class DeleteBuilder
      implements PromiseLike<{ error: null }>
    {
        ...
        from(bucket: string) {
          return {
            async remove(paths: string[]) {
              ...
              const response: DatabaseResponse<unknown> = { data: null, error: null };
              return response;
            },
          };
        },
    ...
    expect(storageRemovals).toEqual([
      { bucket: "images", paths: ["user-1/asset-1.png"] },
    ]);
  ...
  it("moves render job output to assets", async () => {
    ...
    const signedRequests: Array<{ bucket: string; path: string; expiresIn: number }> = [];
    ...
    const fetchImpl = async (input: RequestInfo | URL, init?: RequestInit) => {
      fetchCalls.push({ url: formatRequestInfo(input), method: init?.method ?? "GET" });
    ...
    const result = await service.moveRenderJobToAssets({
      userId: "user-1",
      input: { renderJobId: "job-1", originalName: "My Asset.png", metadata: {} },
    });
    ...
    expect(storageLists).toEqual([
      { bucket: "images", path: "user" },
    ]);
    expect(signedRequests).toEqual([]);
    expect(fetchCalls).toEqual([]);
    expect(insertRecords).toHaveLength(1);
    expect(quotaUpdates).toEqual([
      { fileSize: 150, mimeType: "image/png" },
    ]);
  });
});
