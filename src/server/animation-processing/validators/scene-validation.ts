...
export function validateConnectionsGracefully(
  ...
  for (const edge of edges) {
    ...
    const sourcePort = sourceDef.ports.outputs.find((port) => port.id === edge.sourceHandle);
    const targetPort = targetDef.ports.inputs.find((port) => port.id === edge.targetHandle);
