...
describe('media executor batch overrides', () => {
  it('emits per-batch asset overrides and resolves them during scene build', async () => {
    ...
    expect(metadata?.perObjectBatchOverrides?.[imageObject.id]?.['Media.imageAssetId']).toEqual({
      batch1: 'asset-1',
      batch2: 'asset-2',
    });
    ...
    for (const sub of subPartitions) {
      ...
      const scene = await buildAnimationSceneFromPartition(namespaced, fakeAssetCache);
