  ...
  private async executeInsert(
    ...
    for (const input of inputs) {
      ...
      for (const objectDef of inputData) {
        if (typeof objectDef === 'object' && objectDef !== null) {
          const objectId = (objectDef as Record<string, unknown>).id as string | undefined;
          ...
          const perObjectBoundVal = getNumber(readVarForObject(objectId)('appearanceTime'));
        ...
        if (upstream && typeof upstream === 'object') {
          for (const [objectId, fields] of Object.entries(upstream)) {
            if (typeof fields === 'object' && fields !== null) {
              ...
              for (const [fieldPath, byKey] of Object.entries(fields as Record<string, unknown>)) {
    ...
    const mergedPerObjectBoundFields: Record<string, string[]> | undefined = (() => {
      ...
      for (const obj of timedObjects) {
        ...
        const objectId = (obj as Record<string, unknown>).id as string | undefined;
        ...
        if (globalBound || perObjBound) {
          ...
          out[objectId] = Array.from(new Set([...existing, 'appearanceTime']));
        }
      }
