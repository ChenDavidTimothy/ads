...
export class ExecutionEngine {
  ...
  // PERFORMANCE OPTIMIZATION: Targeted execution method (reuses debug pattern)
  async executeFlowToTarget(
    nodes: ReactFlowNode<NodeData>[], 
    edges: ReactFlowEdge[], 
    targetNodeId: string,
    options?: { requireScene?: boolean }
  ): Promise<ExecutionContext> {
    logger.debug(`Starting targeted execution to node: ${targetNodeId}`);
    
    // PERFORMANCE OPTIMIZATION: Use appropriate validation level
    ...
    const context = createExecutionContext();
    context.debugMode = true; // Enable for targeted execution
    context.debugTargetNodeId = targetNodeId;
    
    // PERFORMANCE OPTIMIZATION: Filter edges once
    const dataEdges = edges.filter((e) => (e.kind ?? 'data') === 'data');
    const executionOrder = getTopologicalOrder(nodes, dataEdges);
    
    // PERFORMANCE OPTIMIZATION: Find target index once
    const targetNodeIndex = executionOrder.findIndex(n => n.data.identifier.id === targetNodeId);
    if (targetNodeIndex === -1) {
      throw new Error(`Target node ${targetNodeId} not found in execution order`);
    }
    
    // PERFORMANCE CORE: Execute only required nodes (subgraph execution)
    const nodesToExecute = executionOrder.slice(0, targetNodeIndex + 1);
    
    logger.debug(`Executing ${nodesToExecute.length}/${executionOrder.length} nodes for target`, { 
      target: targetNodeId,
      efficiency: `${Math.round((1 - nodesToExecute.length / executionOrder.length) * 100)}% nodes skipped`,
      executingNodes: nodesToExecute.map(n => ({
        id: n.data.identifier.id,
        type: n.type,
        displayName: n.data.identifier.displayName
      }))
    ...
    // PERFORMANCE OPTIMIZATION: Execute minimal required set
    for (const node of nodesToExecute) {
      if (!isNodeExecuted(context, node.data.identifier.id)) {
        logger.debug(`Executing: ${node.data.identifier.displayName} (${node.type})`);
        
        // Skip conditional routing check for performance in individual generation
        if (shouldSkipDueToConditionalRouting(node, context, nodes, dataEdges)) {
          logger.debug(`Skipping due to conditional routing: ${node.data.identifier.displayName}`);
          continue;
        }

        // Runtime validation only for non-merge nodes
        if (node.type !== 'merge') {
          this.validateNoDuplicateObjectsAtRuntime(node, context, dataEdges);
        }
        ...
        if (executor) {
          await executor.execute(node, context, edges);
          markNodeExecuted(context, node.data.identifier.id);
          logger.debug(`Completed: ${node.data.identifier.displayName}`);
        } else {
          logger.warn(`No executor found for node type: ${node.type}`);
        }
      }
    ...
    logger.debug('Targeted execution completed', {
      target: targetNodeId,
      nodesExecuted: context.executedNodes.size,
      scenesWithObjects: context.sceneObjectsByScene.size,
      totalAnimations: context.sceneAnimations.length,
      efficiency: `${Math.round((1 - nodesToExecute.length / executionOrder.length) * 100)}% computation saved`
    });
