...
export function extractDomainError(error: unknown): { code: DomainErrorCode; details?: DomainErrorDetails; message?: string } | null {
  ...
  const details = isRecord(rawDetails) ? (rawDetails as DomainErrorDetails) : undefined;

  const message = (error as { message?: unknown }).message;
  return { code: errorCode as DomainErrorCode, details, message: typeof message === 'string' ? message : undefined };
}
