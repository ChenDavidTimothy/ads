...
import { sanitizeForFilename, buildContentBasename, buildFilename } from '../naming';

describe('Filename Sanitization', () => {
  describe('sanitizeForFilename', () => {
    it('should replace illegal characters with dashes', () => {
      expect(sanitizeForFilename('file\\with/invalid:chars*"<>|')).toBe('file-with-invalid-chars');
    });
    ...
    it('should handle null bytes and control characters', () => {
      expect(sanitizeForFilename('file\x00with\ncontrol\rchars\t')).toBe('file-with-control-chars');
    });

    it('should handle vertical tab and form feed', () => {
      expect(sanitizeForFilename('file\x0bwith\x0cchars')).toBe('file-with-chars');
    });
  ...
  describe('buildContentBasename', () => {
    ...
    it('should sanitize both display name and batch key', () => {
      expect(buildContentBasename('Scene\\:*?"<>|', 'batch\\:*?"<>|')).toBe('Scene-batch');
    });
  ...
  describe('buildFilename', () => {
    ...
    it('should include batch key in filename', () => {
      expect(buildFilename('My Scene', '.mp4', 'batch1')).toBe('My-Scene-batch1.mp4');
    });

    it('should sanitize all components', () => {
      expect(buildFilename('Scene\\:*?"<>|', '.mp4', 'batch\\:*?"<>|')).toBe('Scene-batch.mp4');
    });
  });
