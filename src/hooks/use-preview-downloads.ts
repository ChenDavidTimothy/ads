...
import { downloadFile, downloadFilesAsZip, type DownloadableFile, generateSafeFilename } from '@/utils/download-utils';
...
export function usePreviewDownloads({ videos = [], images = [] }: UsePreviewDownloadsProps) {
    ...
    try {
      await downloadFile(
        ...
        },
        {
          ...
          onComplete: () => {
            toast.success('Download Complete', `${video.sceneName} has been downloaded`);
            setDownloadProgress(0);
            setCurrentFile('');
          },
          onError: (error) => {
    ...
    try {
      await downloadFile(
        ...
        },
        {
          onProgress: (progress) => setDownloadProgress(progress),
          onComplete: () => {
            toast.success('Download Complete', `${image.frameName} has been downloaded`);
            setDownloadProgress(0);
            setCurrentFile('');
          },
          onError: (error) => {
    ...
    try {
      await downloadFilesAsZip(
        files,
        {
          ...
          onComplete: () => {
            toast.success('Download Complete', `All ${completedVideos.length} videos have been downloaded as ZIP`);
            setDownloadProgress(0);
            setCurrentFile('');
          },
          onError: (error, file) => {
    ...
    try {
      await downloadFilesAsZip(
        files,
        {
          ...
          onComplete: () => {
            toast.success('Download Complete', `All ${completedImages.length} images have been downloaded as ZIP`);
            setDownloadProgress(0);
            setCurrentFile('');
          },
          onError: (error, file) => {
    ...
    try {
      await downloadFilesAsZip(
        allFiles,
        {
          ...
          onComplete: () => {
            toast.success('Download Complete', `All content (${allFiles.length} files) has been downloaded as ZIP`);
            setDownloadProgress(0);
            setCurrentFile('');
          },
          onError: (error, file) => {
