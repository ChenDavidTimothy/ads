...
interface TabEntry { id: string; lastSeen: number }
...
export function useMultiTabDetection(workspaceId: string) {
  ...
  useEffect(() => {
    ...
    const upsert = () => {
      const now = Date.now();
      const filtered = readTabs(storageKey).filter((t) => now - t.lastSeen < STALE_MS);
      const updated = [...filtered.filter((t) => t.id !== tabId), { id: tabId, lastSeen: now }];
    ...
    const onStorage = (e: StorageEvent) => { if (e.key === storageKey) upsert(); };
    ...
    return () => {
      ...
      const now = Date.now();
      const filtered = readTabs(storageKey).filter((t) => t.id !== tabId && now - t.lastSeen < STALE_MS);
