...
export function AuthStatus() {
  ...
  const scheduleSessionRefresh = useCallback(
    (expiresAt: Date) => {
      ...
      console.log(
        `[AUTH] Scheduling session refresh in ${Math.round(refreshTime / 1000)} seconds`,
      );
            ...
            if (error) {
              ...
              showToastOnce("refresh-failed", () => {
                toast.warning(
                  ...
                  "Please log in again if you experience issues",
                );
              });
    ...
    try {
      ...
      if (data.session?.user) {
        ...
        if (expiresAt.getTime() - now.getTime() < 60 * 1000) {
          console.log(
            "[AUTH] Session expired or about to expire, refreshing...",
          );
          ...
          const newExpiresAt = new Date(
            (refreshData.session.expires_at ?? Date.now() / 1000 + 3600) * 1000,
          );
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (justSignedIn) {
        // Remove the cookie
        document.cookie =
          "justSignedIn=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
        ...
        case "SIGNED_IN":
          if (session?.user) {
            ...
            const expiresAt = new Date(
              (session.expires_at ?? Date.now() / 1000 + 3600) * 1000,
            );
            setSessionExpiresAt(expiresAt);
            scheduleSessionRefresh(expiresAt);
          }
  ...
  const getSessionTimeRemaining = useCallback(() => {
    ...
    if (timeRemaining < 30 * 60 * 1000)
      return `${Math.round(timeRemaining / (60 * 1000))}m remaining`;
  ...
  if (authState === "error") {
    return (
      <div className="flex items-center gap-[var(--space-2)] text-sm text-[var(--danger-500)]">
    ...
    return (
      <div className="flex items-center gap-[var(--space-3)] text-sm">
        <div className="flex flex-col">
          ...
          {timeRemaining && (
            <span
              className={`text-xs ${timeRemaining === "Expiring soon" ? "text-[var(--warning-600)]" : "text-[var(--text-tertiary)]"}`}
            >
