...
export function SaveStatus({
  ...
  const status = (() => {
    ...
    return {
      icon: CheckCircle2,
      label: lastSaved ? `Saved ${formatAgo(lastSaved)}` : "Saved",
  ...
  return (
    ...
    >
      ...
      {hasMultipleTabs && (
        <span className="ml-2 flex items-center gap-1 text-[var(--warning-600)]">
          <AlertTriangle size={14} /> Multi-tab
        </span>
      )}
    </div>
  );
...
function tooltip(args: {
  ...
  if (args.lastSaved)
    parts.push(`Last saved: ${args.lastSaved.toLocaleString()}`);
