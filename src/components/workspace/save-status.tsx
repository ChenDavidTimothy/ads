...
export function SaveStatus({ lastSaved, hasUnsavedChanges, isSaving, isOnline = true, hasBackup = false, hasMultipleTabs = false }: SaveStatusProps) {
  const status = (() => {
    if (!isOnline) return { icon: WifiOff, label: 'Offline', className: 'text-yellow-400' };
    if (isSaving) return { icon: HardDriveDownload, label: 'Saving…', className: 'text-blue-400 animate-pulse' };
    if (hasUnsavedChanges) return { icon: Clock4, label: 'Unsaved changes', className: 'text-amber-400' };
    return { icon: CheckCircle2, label: lastSaved ? `Saved ${formatAgo(lastSaved)}` : 'Saved', className: 'text-emerald-400' };
  })();
  ...
  return (
    <div className={cn('flex items-center gap-2 text-sm', status.className)} title={tooltip({ lastSaved, hasBackup, hasMultipleTabs, isOnline, hasUnsavedChanges })}>
      ...
      {hasBackup && <span className="ml-1 text-xs text-gray-400">(backup)</span>}
      {hasMultipleTabs && <span className="ml-2 text-yellow-400 flex items-center gap-1"><AlertTriangle size={14} /> Multi-tab</span>}
    </div>
  );
}
