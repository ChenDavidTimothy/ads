...
export function PreviewPanel({ videoUrl, videos, imageUrl, images = [] }: PreviewPanelProps) {
  ...
  if (!hasMultipleVideos && !videoUrl && !hasSingleImage && !hasMultipleImages) {
    return (
      <div className="py-[var(--space-8)] text-center text-[var(--text-tertiary)]">
        <div className="text-refined mb-[var(--space-2)] text-sm">No preview available</div>
        <div className="text-refined text-xs">Generate content to see preview</div>
      </div>
    );
  }
