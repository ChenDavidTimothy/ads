    ...
    onSuccess: (data) => {
      ...
      if ('immediateResult' in data && data.immediateResult) {
        const videoJob: VideoJob = {
          ...
          sceneName: buildContentBasename(data.immediateResult.nodeName, undefined),
  ...
  const generateImage = api.animation.generateImage.useMutation({
    ...
    onSuccess: async (data) => {
      ...
      if ('immediateResult' in data && data.immediateResult) {
        const job: ImageJob = {
          jobId: data.immediateResult.jobId,
          frameName: buildContentBasename(data.immediateResult.nodeName, undefined),
        ...
        try {
          ...
          setImages((prev) =>
            prev.map((img) => {
              ...
              if (!s) return img;
              const newStatus = (
                s.status === 'completed' ? 'completed' : s.status === 'failed' ? 'failed' : s.status
              ) as ImageJob['status'];
          ...
          res.forEach((s) => {
            if (s.status === 'completed' || s.status === 'failed') pending.delete(s.jobId);
          });
