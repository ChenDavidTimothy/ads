...
export function CanvasEditorTab({ nodeId }: { nodeId: string }) {
  ...
  React.useEffect(() => {
    console.log(
      `[Canvas] Detected ${upstreamObjects.length} objects for canvas node ${nodeId}:`,
  ...
  const handleUpdateAssignment = useCallback(
    (updates: Record<string, unknown>) => {
      ...
      if (
        ...
        mergedInitial.scale = {
          ...(baseInitial.scale as Record<string, unknown>),
          ...(updates.scale as Record<string, unknown>),
        };
      }
      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="flex-1 p-[var(--space-4)]">
          <div className="flex h-full w-full items-center justify-center text-[var(--text-tertiary)]">
            No timeline for Canvas. Select Default or an object on the left to
            edit its properties.
          </div>
        </div>
      </div>
...
function CanvasPerObjectProperties({
  nodeId,
  objectId,
  assignments,
  onChange,
