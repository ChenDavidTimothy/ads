    ...
    <Card className={`p-4 min-w-[200px] ${getNodeHeight()} ${selected ? 'ring-2 ring-blue-500' : ''}`}>
      ...
      <CardHeader className="p-0 pb-3">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 bg-teal-600 flex items-center justify-center rounded text-white font-bold text-sm">
            ⊕
          </div>
      ...
      <CardContent className="p-0 space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-xs text-gray-300">Ports:</span>
          <span className="text-xs text-white font-medium">{portCount}</span>
        </div>
