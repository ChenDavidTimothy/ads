...
export function useVariableBinding(nodeId: string, objectId?: string) {
  ...
  const variables = useMemo(() => {
    ...
    return tracker.getAvailableResultVariables(
      nodeId,
      state.flow.nodes,
      state.flow.edges,
    );
  ...
  const clearTrackOverride = (
    nextData: AnimationNodeData,
    objectId: string,
    key: string,
  ): void => {
    ...
    const tracks: TrackOverride[] = Array.isArray(entry.tracks)
      ? [...entry.tracks]
      : [];
  ...
  const bind = (key: string, resultNodeId: string): void => {
    updateFlow({
      nodes: state.flow.nodes.map((n) => {
        ...
        // 1) Set the binding
        if (objectId) {
          const prevAll: PerObjectVariableBindings =
  ...
  return (
    <div className={`relative ${className ?? ""}`}>
      <button
        ...
        title={
          boundId
            ? `Bound to ${boundName ?? boundId}`
            : "Bind to Result variable"
        }
        onClick={() => setOpen(true)}
        className={`relative rounded p-1 hover:bg-[var(--surface-interactive)] ${isBound ? "text-[var(--accent-primary)]" : ""}`}
      >
        <LinkIcon size={14} />
        {isBound && (
              ...
              <div className="space-y-[var(--space-1)]">
                {filtered.map((v) => (
                  <button
                    ...
                    className="w-full rounded-[var(--radius-sm)] px-[var(--space-3)] py-[var(--space-2)] text-left text-sm transition-colors duration-[var(--duration-fast)] hover:bg-[var(--surface-interactive)] focus:bg-[var(--surface-interactive)] focus:ring-1 focus:ring-[var(--ring-color)] focus:outline-none"
                  >
