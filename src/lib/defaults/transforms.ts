...
function getTransformShortId(transformType: string): string {
  ...
  if (!transformType || typeof transformType !== "string") {
    console.warn(
      "getTransformShortId called with invalid transformType:",
      transformType,
    );
    return "unk";
  }
...
function generateUniqueTransformDisplayName(
  ...
  const existingNames = new Set(
    existingTracks
      .map(
        (t) =>
          (t as unknown as { identifier?: TransformIdentifier }).identifier
            ?.displayName,
      )
...
export function generateTransformIdentifier(
  ...
  const sameTypeCount = existingTracks.filter(
    (t) => t.type === transformType,
  ).length;
  const sequence = sameTypeCount + 1;
  const suffix = Math.random().toString(36).slice(2, 10);
  const id = `${shortId}_${year}_${sequence.toString().padStart(3, "0")}_${suffix}`;
  const displayName = generateUniqueTransformDisplayName(
    transformType,
    existingTracks,
  );
  return {
...
export function validateTransformDisplayName(
  ...
  const conflict = allTracks.find((t) => {
    const identifier = (t as unknown as { identifier: TransformIdentifier })
      .identifier;
