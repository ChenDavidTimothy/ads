...
class ProductionMonitor {
  ...
  logAuthError(error: Error, context: Partial<AuthErrorContext> = {}): void {
  ...
  private async sendToMonitoringService(event: MonitoringEvent): Promise<void> {
    try {
      ...
      logger.debug('Monitoring event sent to external service', {
        type: event.type,
      });
    } catch (error) {
...
export function setupGlobalErrorHandling(): void {
  ...
  if (typeof window !== 'undefined') {
    ...
    window.addEventListener('error', (event) => {
      monitor.logAPIError(new Error(`Global Error: ${event.message}`), 'global/javascript-error', {
        path: window.location.pathname,
...
export const monitoringHelpers = {
  ...
  timeFunction: async <T>(name: string, fn: () => Promise<T>, threshold?: number): Promise<T> => {
